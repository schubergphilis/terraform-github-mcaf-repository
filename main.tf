...
resource "github_branch_protection" "default" {
  for_each = { for k, v in var.branches : k => v if v.branch_protection != null || v.use_default_branch_protection == true }

  enforce_admins         = try(each.value.branch_protection.enforce_admins, null)
  pattern                = each.key
